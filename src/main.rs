...
fn dump_vec(queue: &[char]) -> String {
    let mut result: String = String::with_capacity(queue.len() * 4 + 6);
    result.push_str("vec![");
    let queue_content = queue
        .iter()
        .map(|ch| {
            let cur = format!("'{}'", ch);
            cur
...
fn sort_entities() -> EntityDisplay {
    ...
    let mut entities: Vec<(&'static str, u32)> = Vec::with_capacity(total);
    // binary search
    ...
    for &(entity, code) in sorted.iter() {
        entities.push((entity, code));
    }
    //
    EntityDisplay {
        positions,
